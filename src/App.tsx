...
function App() {
  ...
  return <main>
    ...
    {tab === 'yukleme' && <section className="panel wide loadPlanner"><div className="productTabs compact"><div className="productTabButtons">{products.map((p,i) => <button key={p.id} className={activeProductIndex===i?'active':''} onClick={() => setActiveProductIndex(i)}>{p.name}</button>)}</div><div className="productHint">Seçili ürünün ölçüleri aşağıdaki fırın hesabına aktarılır.</div></div><span className="eyebrow">FIRIN YÜKLEME</span><h2>Raf ölçüleri</h2><p className="muted">Ürün ölçüleri Çamur sekmesinden otomatik gelir.</p><div className="fields"><Field label="Raf ölçüsü" value={shelfSize} set={setShelfSize} suffix="mm"/><label className="field"><span>Raf aralığı</span><div><select value={shelfGap} onChange={e => setShelfGap(Number(e.target.value))}>{rackGapOptions.map(o => <option key={o.gap} value={o.gap}>{o.gap} mm · {o.label}{o.gap === recommendedRackGap ? ' ★ Önerilen' : ''}</option>)}{!rackGapOptions.some(o => o.gap === shelfGap) && <option value={shelfGap}>{shelfGap} mm · Özel</option>}</select><b>mm</b></div></label></div><div className="dimensionSource"><span>Çamur sekmesinden gelen ürün</span><b>{shape} · Çap {diameter} mm · Yükseklik {height} mm · Genişlik {width} mm</b></div><div className="loadSummary"><div><span>Raf başına yaklaşık</span><b>{shelfCapacity} ürün</b></div><div><span>Ürün grubu</span><b>{products.reduce((sum,p) => sum + p.pieces, 0)} adet</b></div><div><span>Raf aralığı</span><b>{shelfGap} mm</b></div><div><span>Fırın iç yüksekliği</span><b>{kiln.height} mm</b></div></div>
        ...
        <div className="rackRecommendation"><div><span className="eyebrow">RAF ARALIĞI ÖNERİSİ</span><b>{recommendedRackGap} mm</b><small>Ürün yüksekliği + güvenlik payı + adet/fırın kapasitesi birlikte değerlendirildi.</small></div><div><span>Mevcut</span><strong>{shelfGap} mm</strong>{selectedRackOption ? <small>{selectedRackOption.fits ? '✓ Bu ayar adet açısından sığıyor' : '⚠️ Bu ayarda toplam adet kapasiteyi aşıyor'}</small> : <small>Özel değer</small>}</div><div><span>Seviye / kapasite</span><strong>{rackGapOptions.find(x => x.gap===recommendedRackGap)?.requiredLevels || 0} / {rackGapOptions.find(x => x.gap===recommendedRackGap)?.capacity || 0}</strong><small>{rackGapOptions.find(x => x.gap===recommendedRackGap)?.remaining || 0} ürün kapasite payı</small></div></div>
        ...
        <p className="note">Sistem ürünleri taban alanı, yükseklik ve adetlerine göre birlikte değerlendirir. Raf aralığı seçenekleri ürün yüksekliği + güvenlik payı ile oluşturulur; toplam adet ve fırın iç yüksekliği de kapasite kontrolüne dahil edilir. Bu değerler planlama önerisidir. Gerçek yüklemede raf ayağı, raf kalınlığı, ısı dolaşımı ve fırın üreticisinin güvenlik mesafeleri ayrıca kontrol edilmelidir.</p></section>}
