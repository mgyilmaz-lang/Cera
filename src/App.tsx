...
function App() {
  ...
  const shelfCapacity = useMemo(() => {
    const productDiameter = Math.max(20, diameter);
  ...
  return <main>
    ...
    {tab === 'yukleme' && <section className="panel wide loadPlanner"><div className="productTabs compact"><div className="productTabButtons">{products.map((p,i) => <button key={p.id} className={activeProductIndex===i?'active':''} onClick={() => setActiveProductIndex(i)}>{p.name}</button>)}</div><div className="productHint">Seçili ürünün ölçüleri aşağıdaki fırın hesabına aktarılır.</div></div><span className="eyebrow">FIRIN YÜKLEME</span><h2>Raf ölçüleri</h2><p className="muted">Ürün ölçüleri Çamur sekmesinden otomatik gelir.</p><div className="fields"><Field label="Raf ölçüsü" value={shelfSize} set={setShelfSize} suffix="mm"/><Field label="Raf aralığı" value={shelfGap} set={setShelfGap} suffix="mm"/></div><div className="dimensionSource"><span>Çamur sekmesinden gelen ürün</span><b>{shape} · Çap {diameter} mm · Yükseklik {height} mm · Genişlik {width} mm</b></div><div className="loadSummary"><div><span>Raf başına yaklaşık</span><b>{shelfCapacity} ürün</b></div><div><span>Ürün çapı</span><b>{diameter} mm</b></div><div><span>Raf aralığı</span><b>{shelfGap} mm</b></div></div><p className="note">Bu kapasite basit bir yerleşim tahminidir. Kenar boşluğu ve ürünler arası güvenlik mesafesi ayrıca dikkate alınmalıdır.</p></section>}
